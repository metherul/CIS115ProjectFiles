...
else
{
	...
	if (!result)
	{
		die("Could not successfully run query ($dropPersonnelTable) from $db: " .	
			mysqli_error($connect) );
	}else  {
			...
			if (!$result) 
			{
				...
				if (!$result) 
				{
					...
					if (!$result) 
					{
						...
						print("<html><head><title>MySQL Setup</title>
						...
						<strong>test</strong> database.</p><p>You are now ready to work through 
						the MySQL samples and exercises in this textbook.</p></body></html>");
					}
				}
			}
		}
